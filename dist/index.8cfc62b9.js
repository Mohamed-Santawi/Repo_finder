...
const performFunction = (searchTerm, isUserSelection)=>{
    const typeQuery = isUserSelection ? "+type:user" : "+type:org";
    fetch(`${USERS_API}${typeQuery}`).then((result)=>result.json())// then( ( response )=>console.log( response ) )
    .then((response)=>setSearchResult(response.items)).catch((error)=>console.error(new Error(error)));
};
...
btnElement?.addEventListener("click", (e)=>{
    ...
    performFunction(inputElement.value, isUserSelection);
});

},{}]},["86oZd","6rimH"], "6rimH", "parcelRequired873")
